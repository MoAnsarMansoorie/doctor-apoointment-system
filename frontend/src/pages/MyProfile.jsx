...
const MyProfile = () => {
  const [userData, setUserData] = useState({
    ...
    phone: "+1 234 567 890",
    address: {
      line1: "123 Main St, Springfield, USA",
      line2: "Apt 4B",
      city: "Springfield",
    },
    gender: "Male",
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      {
        isEdit
          ? <input className='text-3xl bg-gray-100 font-medium max-w-60 mt-4' type='text' value={userData.name} onChange={e => setUserData(prev => ({ ...prev, name: e.target.value }))} />
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          <p className='font-medium'>Phone:</p>
          {
            isEdit
              ...
              : <h2 className='text-blue-400'>{userData.phone}</h2>
          }
      ...
      <div className='mt-5'>
        {
          isEdit
            ...
            : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all duration-300' onClick={() => setIsEdit(true)}>Edit Information</button>
        }
      </div>
